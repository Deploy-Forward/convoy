...
| --- | --- | --- | --- | --- |
| `grok` | `grok` | `grok -m <model?> --agent <path?> --resume <vendor-id?>` | Writes PATH ungate block; installs `neuron-identity`; writes Convoy-owned `--agent` file; writes project PreToolUse hook (`convoy inbox --hook-pretooluse`). | Native CLI on PATH. Named live seats queue (`delivery: queued`); `convoy grok-acp` is the vendor session-message API (`session/prompt` over ACP). Idle TUI wake needs a shared grok leader; without one, inbox + PreToolUse stays deferred. Never steals `--resume`. Never grok `-p`/`-c` as delivery. |
