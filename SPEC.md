...
**Status of this tree:** work lives at `/workspace/convoy` on the Grok Bot box. Not a landed GitHub checkout yet. Do not clone a git repo to "restore" this file.
...
Today's working wire (customer 1): Shell on Aether-Deployed `machineId` `64a3fdd5-2c54-4038-8984-019382b68a78` running `C:\.grok\Invoke-AgentChannel.ps1` and `C:\.grok\ConvoyLayer.ps1` wrapping `ola-brain.exe`. MCP catalog has no Convoy plugin. Status **RED** for HTTP MCP, **GREEN** for PC CLI hop.

The copy of `ConvoyLayer.ps1` in this tree (`/workspace/convoy/ConvoyLayer.ps1`) is the same contract: `hook`, `feed-since`, `send-dry`. Live hops on Aether still go through ola-brain. Default `python -m convoy send` uses a fake runner. Pass `--live` to exec ola-brain. HTTP MCP server code is in `src/convoy/mcp_http.py` (`python -m convoy mcp --root ROOT --port 8788`). Grok Bot catalog attach is still **RED**. Do not claim `https://convoy.bot/mcp` is live.
...
Skill on disk: /home/<user>/agent-data/workflows/agent-channel/SKILL.md
```

If a field is unknown, write `unknown` or JSON `null`. Do not fill it from memory.

### Definition of done (MCP attach)

Grok Bot attaches HTTP MCP from catalog or URL. `roster` returns live agents with nulls not guesses. `send grok` returns a `SYNAPSE_OK`-class compact body. `feed --since` works from this chat without Shell paste. Until that attach happens, this section stays RED for HTTP MCP.
...
| 7 | Durable convoy_id / attach / bring-up | bind+attach stamps GREEN 2026-08-28 `cvy_<thread-a>` thread `customer1`; resume hop RED silent hang; bring-up dry-run unit this fold; live TUI RED. Not Phase 8. |

MCP attach rides with Phase 1 send/context. HTTP MCP stays RED until this chat can call it without Shell paste. That does not unlock Phase 2 by itself.
...
- **GREEN Aether:** convoy hook stamps `{ts,kind,instance_id,summary}` to `C:\Users\<user>\ola\da-integration\.convoy\feed.jsonl` via `C:\.grok\ConvoyLayer.ps1`. `convoy feed --since` returns that window. Example c1-locked ts `2026-08-28T14:42:46.975866Z`.
...
- **RED:** `cursor-agent status` logged in `dummy@example.com`, no remaining quota in `status` / `about`.
...
- **GREEN live isolated n-pane TDD (2026-08-29 ~4:21–4:23 PM ET):** `C:\\Users\\<user>\\ola\\client-test\\.convoy\\tdd-panes.jsonl`. One new CASCADIA per combo, splits inherited: n=2 grok+grok, n=2 claude+grok, n=3 claude+grok+grok, n=2 claude+claude. `C:\\` hwnd 67496 untouched. `--version`, `-w <name>`, and `--` before exe popped WT Help 1.24.11911.0 (RED, dialog closed).
...
| HTTP MCP server | `src/convoy/mcp_http.py` JSON-RPC POST `/mcp`. Attach from this chat still **RED**. |
...
- Claim HTTP MCP GREEN while the catalog has no Convoy plugin and this chat is not attached.
