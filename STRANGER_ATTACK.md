...
- `convoy_id`: `cvy_<thread-a>`
- `thread`: `convoy test`
- checkout: `C:\Users\<user>\ola\da-integration` (`integration/convoy-web-poc-20260828`, PR 167)
...
Live `terminals` / `bring_up` with `cvy_<thread-c>`:
...
| agy | `worktree=C:\Users\<user>\ola\client-test` | fake `ok`, `session_id=spawned-agy`, body `ACK agy: …` |
...
What is actually live: a Cloudflare-fronted 7-tool HTTP MCP, one process, one `cvy_<thread-a>` / `convoy test` / da-integration checkout, fake `send`, seated grok/claude refused, onboard/install/hide/glance absent, Python urllib 1010 without a Chrome signature.
