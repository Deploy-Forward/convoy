(function () {
  ...
  function bootHonestyCarousel() {
    const scroller = document.getElementById("honesty-scroller");
    const prevButton = document.getElementById("honesty-prev");
    const nextButton = document.getElementById("honesty-next");
    const live = document.getElementById("honesty-live");
    if (!scroller || !prevButton || !nextButton || !live) {
      return;
    }

    const slides = Array.from(scroller.querySelectorAll(".honesty-slide"));
    const dots = Array.from(document.querySelectorAll(".honesty-dot"));
    if (!slides.length) {
      return;
    }
    ...
    function nearestIndex() {
      ...
      for (let i = 0; i < slides.length; i += 1) {
        const delta = Math.abs(slides[i].offsetLeft - target);
    ...
    function refreshState(index) {
      ...
      activeIndex = bounded;
      prevButton.disabled = bounded === 0;
      nextButton.disabled = bounded === slides.length - 1;
      const word = slides[bounded].getAttribute("data-word") || `Slide ${bounded + 1}`;
      live.textContent = `Slide ${bounded + 1} of ${slides.length}: ${word}.`;
    ...
    const supportsSnapChange = "onscrollsnapchange" in scroller || "SnapEvent" in window;
