...
class Phase7Hide(unittest.TestCase):
    ...
    def test_live_argv_refuses_ola_brain_and_dash_d_wrap(self):
        ...
        with self.assertRaises(ValueError):
            _live_argv(["claude", "-d", r"C:\Users\<user>\ola\ola-brain", "--", abs_claude, "--resume", "sess-claude"])
        with self.assertRaises(ValueError):
            _live_argv(["wt", "-d", r"C:\tmp", "--", abs_claude, "--resume", "sess-claude"])
        with self.assertRaises(ValueError):
            _live_argv(["claude", "--resume", "sess-claude"])
        out = _live_argv([wt, "--window", "new", "nt", "--title", "claude-0", "-d", r"C:\Users\<user>\ola\ola-brain", abs_claude, "--resume", "sess-claude"])
